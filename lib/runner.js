...
Runner.prototype.compile = function(test) {
    if (/Error$/.test(test.attrs.negative)) {
      test.contents = [
        "assert.throws(" + test.attrs.negative + ", function() {",
        "  eval(" + JSON.stringify("throw NotEarlyError;" + test.contents) + ");",
        "});"
      ].join("\n");
      delete test.attrs.negative;
    }
