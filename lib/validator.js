module.exports = function validate(test) {
  ...
  const ranToFinish = result.stdout.indexOf('Test262:AsyncTestComplete') > -1 ||
	result.stdout.indexOf('Test262-harness:SyncTestComplete') > -1;
  ...
  if (!isNegative) {
    if (result.error !== null) {
      ...
      let message = result.stderr ? result.stderr : 'Test did not run to completion';
      return {
        pass: false,
        message,
      };
